...
async function sendInvoiceEmail(toEmail, data, isPaid = false) {
    ...
    const title = isPaid ? "Konfirmasi Pembayaran Lunas" : "Invoice Tagihan";
    const statusNote = isPaid ? "Pembayaran Telah Diterima" : "Menunggu Pembayaran";
    ...
    const mailOptions = {
        ...
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 500px; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
                <h2 style="color: #24b3ae; text-align: center;">${title}</h2>
                <p>Halo <b>${data.nama}</b>,</p>
                <p>Status: <span style="font-weight:bold; color:${isPaid ? '#28a745' : '#ffc107'}">${statusNote}</span></p>
                <hr>
                ...
                ${!isValidEmail(toEmail) ? `<p style="color:red; font-size:11px;">*Email user tidak valid, invoice dikirim ke default admin.</p>` : ''}
            </div>`
    };
